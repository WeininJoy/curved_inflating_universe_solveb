###### Goal: modify primpy/events.py by adding new event -- SlowRowEvent
...
class SlowRowEvent(Event):
    ...
    def __call__(self, x, y):
        ...
        return self.value - self.equations.w(x, y) 

class KineticDominanceEvent(Event):
    """Track kinetic dominance start."""

    def __init__(self, equations, direction=0, terminal=False, value=0.99, **kwargs):
        super(KineticDominanceEvent, self).__init__(equations, direction, terminal, value)
        self.name = 'KineticDominance_dir%d_term%d' % (self.direction, self.terminal)
