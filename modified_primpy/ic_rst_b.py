...
class IC_RST_b(object):
    def __init__(self, background, cs=1, w=-0.99):
        self.background = background  # the equation should have track_b=True
        ...
        if -1. < self.w < -1./3.: # match a&b in inflation era
            ...
            if self.backwards.t[-1] < self.background.t[0]* (1.+1.e-2): # make sure get R_IC at the start of inflation
                ...
                self.dNbdt_i = self.backwards.dNbdt[-1]
            else: 
                print("Cannot get R_IC at the start of inflation.")

        elif -1./3. < self.w <= 1.: # match a&b in kinetic dominance era
            ...
            ic = InflationStartIC(equations=eq, phi_i=background.phi[0], N_i=background.N[0], t_i=self.background.t[0], eta_i=0, x_end=-1.e10)
            ...
            self.forward = solve(ic=ic_KD, events=ev)
            if self.forward.t[-1] > self.background.t[0]* (1.-1.e-2): # make sure get R_IC at the start of inflation
                self.Nb_i = self.forward.Nb[-1]
                self.dNbdt_i = self.forward.dNbdt[-1]
            else: 
                print("Cannot get R_IC at the start of inflation.")

        elif self.w == -1./3.: # match a&b at the start of inflation
            phi, dphidt, N = self.background.phi[0], self.background.dphidt[0], self.background.N[0]
            V = self.V(phi)
            dNdt = np.sqrt((dphidt**2 /2 + V) /3 - self.K * np.exp(-2*N))
            self.Nb_i = N
            self.dNbdt_i = dNdt
    ...
    def get_R_IC(self, k):
        # Return IC of R
        K, phi, N, Nb, dNb = self.K, self.background.phi[0], self.background.N[0], self.Nb_i, self.dNbdt_i 
        b = np.exp(Nb)
        db = b* dNb
