...
from primpy.events import UntilNEvent, InflationEvent, KineticDominanceEvent
...
class KD_IC(object): # set IC in Kinetic dominance
    ...
    def __call__(self, y0, **ivp_kwargs):
        ...
        # #########################################################################
        KD_key = 'KineticDominance_dir1_term0'
        # for i in range(len(self.background.t)-1):
        #     if self.background.t[i+1] < 0.1* self.background.t_events[KD_key][0] + 0.9*self.background.t[-1] < self.background.t[i]:
        #         deep_KD_idx = i
        deep_KD_idx = -6
        # self.x_ini = self.background.t_events[KD_key][0]
        self.x_ini = self.background.t[deep_KD_idx]
        self.x_end = self.background.t[0]

        # KD_phi, KD_dphidt, KD_N, KD_eta = self.background.y_events[KD_key][0]
        KD_phi, KD_dphidt, KD_N = self.background.phi[deep_KD_idx], self.background.dphidt[deep_KD_idx], self.background.N[deep_KD_idx]
...
class IC_RST_diff_t(object):
    def __init__(self, background, w, k, cs=1):
        ...
        if -1./3.*(1.+1.e-2) < self.w < -1./3.*(1.-1.e-2): # set at the start of inflation
            ...
            ev = [KineticDominanceEvent(eq, +1, terminal=False, value=self.w)]
    ...
    # #########################################################################
    # define IC of R and dRdt of flat RSET 
    # #########################################################################
