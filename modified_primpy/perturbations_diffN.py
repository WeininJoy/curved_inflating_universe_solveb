###### Goal: modify primpy/time/perturbations.py by setting new quantum IC (based on b)
...
from primpy.time.ic_rst_diff_N import IC_RST_diff_N
...
class ScalarModeT(ScalarMode):
    ...
    def get_vacuum_ic_RST(self):
        ...
        IC_diff_N = IC_RST_diff_N(background = self.background, N_qIC=self.N_qIC, k=self.k)
        # if 0.04 < self.k/self.background.a0_Mpc < 0.06:
        #     IC_diff_t.plot_R()
        Rk_i, dRk_i = IC_diff_N.get_R_IC()
        return Rk_i, dRk_i
